...
class AggContext2(groupByConfig: GroupByConfig) {
  ...
  lazy val getListAggregatorJNode: List[Aggregation2[_]] = createAggColumnList()
  lazy val getListAggregator: List[Aggregation2[_]] = getListAggregatorJNode
  ...
  def createAggColumnList(): List[Aggregation2[_]] = {

    @tailrec
    def iterateThroughLists(l0: List[String],l1: List[String], l2: List[String], l3: List[Aggregation2[_]]):List[Aggregation2[_]] = (l0,l1,l2) match {
      case(_,Nil, Nil) => l3
      case(hl0::tl0, hl1::tl1, hl2::tl2) => iterateThroughLists(tl0, tl1, tl2, l3 :+ Aggregation2(hl2.trim, hl1.trim, hl0.trim))
      case _ =>  throw new Exception(AggContext2.getExceptionMessage("config"))
    ...
    iterateThroughLists(getAliases
      ...
      , List.empty[Aggregation2[_]]) :+ Aggregation2("json", "Key=" + getListGroupByColName.toString, "Key=" + getListGroupByColName.toString )
   ...
   */
  def serializeFromListAggToListByteArray(aggVals: List[_], aggColsIn: List[Aggregation2[_]] = this.getListAggregator) = {

    @tailrec
    def iterateListAggToListBA(aggVals: List[_]
                               , aggColsIn: List[Aggregation2[_]] = this.getListAggregator
  ...
  def deserializeFromListByteArrayToListAgg(isAgg: Boolean, aggVals: List[Array[Byte]], aggCols: List[Aggregation2[_]] = this.getListAggregator) = {

    @tailrec
    def iterateListByteArray(isAgg: Boolean
                             , aggVals: List[Array[Byte]]
                             , aggCols: List[Aggregation2[_]]
  ...
  def initialValues(aggColsIn: List[Aggregation2[_]] = this.getListAggregator) : List[_] = {

    @tailrec
    def iterateAggInitVals(aggColsIn: List[Aggregation2[_]] = this.getListAggregator, acc: List[_] = List.empty[Any]): List[_] = aggColsIn match {
      case Nil if (acc.length > 0) => acc
      case Nil => throw new RuntimeException(AggContext2.getExceptionMessage("empty-config"))
  ...
  */
  ...
  def aggregate(aggType: String)
    ...
    def processAggregate(valList: List[Array[Byte]]
                         ...
                         , aggColList: List[Aggregation2[_]]
  ...
  def testAgg = aggregate(VALUE_MAPPER_TYPE)("Dummy Key",_:Array[Byte],Array.empty[Byte])
}
...
object Aggregation2 {
  def apply[T](aggType:String, aggColumn: String, alias:String) = aggType match {
    case a if a matches "(?i)sum" => new SumAggregation2(aggColumn,alias)
    case a if a matches "(?i)max" => new MaxAggregation2(aggColumn,alias)
    case a if a matches "(?i)min" => new MinAggregation2(aggColumn,alias)
    case a if a matches "(?i)concat" => new ConcatAggregation2(aggColumn,alias)
    case a if a matches "(?i)avg" => new AvgAggregation2(aggColumn,alias)
    case a if a matches "(?i)json" => new JsonAggregation2(aggColumn,alias)
    case a if a matches "(?i)var" => new VarianceAggregation2(aggColumn,alias)
    case a => throw new Exception(a + " - " + AggContext2.getExceptionMessage("unknown-config"))
  } //end apply
} //end Aggregation

abstract class Aggregation2[T](val aggType: String, val aggColumn: String, val alias: String)  {
  type InitType = T
...
abstract class DoubleAggregation2(aggType: String, aggColumn: String, alias:String) extends Aggregation2[Double](aggType, aggColumn, alias) {
  override type InitType = Double
  def convertFn: String => InitType = _.toLong.toDouble
}


abstract class StringAggregation2(aggType: String, aggColumn: String, alias: String) extends Aggregation2[String](aggType, aggColumn, alias) {
  override type InitType = String
  def convertFn: String => InitType = v => v
}

class SumAggregation2(aggColumn: String, alias: String) extends DoubleAggregation2(aggType = "sum", aggColumn, alias) {
...
class MaxAggregation2(aggColumn: String, alias: String) extends StringAggregation2(aggType = "max", aggColumn, alias)  {
...
class MinAggregation2(aggColumn: String, alias: String) extends StringAggregation2(aggType = "min", aggColumn, alias) {
...
class ConcatAggregation2(aggColumn: String, alias: String) extends StringAggregation2("concat", aggColumn, alias) {
  def initializationValue: InitType = ""
...
class AvgAggregation2(aggColumn: String, alias: String)extends StringAggregation2("avg", aggColumn, alias) {
...
class JsonAggregation2(aggColumn: String, alias: String) extends StringAggregation2(aggType = "json", aggColumn, alias) {
  def initializationValue: InitType = ""
...
class VarianceAggregation2(aggColumn: String, alias: String) extends StringAggregation2("var", aggColumn, alias) {
