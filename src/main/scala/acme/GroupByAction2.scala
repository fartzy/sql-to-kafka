...
object GroupByAction2 {
  ...
  def main(args: Array[String]) {
    ...
    val thisAgg = AggContext2.instance(groupByConfig)
    ...
    val result = ktable.filter((k, v) => {
      val t = "";
      val valueNew = thisAgg.getListValsToAgg(v)._2
      valueNew.head == "AD"
     },
...
//          thisAgg.aggregateAdder("key",valueNew, Array.empty[Byte] )
//        )
//      )
